...
public abstract class Conta implements IConta {
    ...
    public void receberTransferencia(double valor, Conta contaOrigem) throws EntradaInvalidaException{
        if(valor < 0 ) throw new EntradaInvalidaException("Entrada inválida");
            ...
            adicionarExtrato(RECEBER_TRANSFERENCIA,valor,contaOrigem);
        }
    }
    ...
    public void sacar(double valor) throws SaldoInsuficienteException, EntradaInvalidaException {
        if( valor < 0){
            ...
            adicionarExtrato(SAQUE,valor,null);
        }
    }
    ...
    public void depositar(double valor) throws EntradaInvalidaException {
        if (valor > 0){
            this.setSaldo(getSaldo() + valor);
            adicionarExtrato(DEPOSITO,valor,null);
        }else
            throw new EntradaInvalidaException("Entrada inválida");
    }
...
//            sacar(valor);
            ...
            adicionarExtrato(TRANSFERENCIA,valor,destino);
    ...
    public void adicionarExtrato(Operacoes operacao, double valor, Conta contaDestino){
        switch (operacao) {
            case TRANSFERENCIA -> extrato.add(new Operacao(-valor,saldo-valor, saldo, TRANSFERENCIA, contaDestino.getNumero()));
            case SAQUE -> extrato.add(new Operacao(-valor,(saldo + valor), saldo, SAQUE));
            case DEPOSITO -> extrato.add(new Operacao(valor,(saldo-valor), saldo, DEPOSITO));
            case RECEBER_TRANSFERENCIA -> extrato.add(new Operacao(valor,saldo - valor,saldo,RECEBER_TRANSFERENCIA,getNumero()));
        }
    }
